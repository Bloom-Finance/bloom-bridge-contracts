...
async function main() {
  ...
  const targetRecepient = Buffer.from(
    tryNativeToHexString(
      '0xF274800E82717D38d2e2ffe18A4C6489a50C5Add', // The address of the user that want to be bridge
      'polygon'
    ),
    'hex'
  );
  ...
  const completeTransfer = await (
    await bloomBridge.completeTransfer(Buffer.from(vaaBytes.vaaBytes, 'base64'))
  ).wait();
